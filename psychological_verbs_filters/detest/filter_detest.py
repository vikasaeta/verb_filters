...
lineList = [line for line in open(r'results_detest.tsv', 'r', encoding='utf-8')]
...
for s in range (1,len(lineList)):
    ...
    if not (fl.isGerund (my_split) or fl.hasGerundAfter(my_split, my_tokens) or fl.isAdjectiveOrNoun(my_split, my_tokens, tagged_tokens, formlist) or fl.hasPattern(my_split[7].lower(), "\s(much|most|widely|her)\sdetested")):
