...
lineList = [line for line in open(r'results_discontinue.tsv', 'r', encoding='utf-8')]

f = open(r"sample_discontinue.tsv", "w",encoding='utf-8')
...
for s in range (1,len(lineList)):
    ...
    formlist = ['discontinued','continuing']
    
    if not (fl.isGerund(my_split) or fl.isAdjective(my_split, my_tokens,tagged_tokens, formlist) or fl.hasGerundAfter(my_split, my_tokens)):
